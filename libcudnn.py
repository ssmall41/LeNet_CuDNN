...
# Removed cudnnGetConvolutionBackwardDataAlgorithm
# Removed cudnnGetConvolutionForwardAlgorithm (Note: still has some other data)
# Removed cudnnGetConvolutionBackwardFilterAlgorithm
# Added cudnnGetConvolutionForwardAlgorithm_v7
# Updated cudnnSetPooling2dDescriptor (it was missing nan prop)
...
_libcudnn.cudnnSetPooling2dDescriptor.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_int,
                                                ...
                                                ctypes.c_int, ctypes.c_int,
                                                ctypes.c_int, ctypes.c_int]
def cudnnSetPooling2dDescriptor(poolingDesc, mode, maxpoolingNanOpt, windowHeight, windowWidth,
    ...
    status = _libcudnn.cudnnSetPooling2dDescriptor(poolingDesc, ctypes.c_int(mode), ctypes.c_int(maxpoolingNanOpt),
                                                   ctypes.c_int(windowHeight), ctypes.c_int(windowWidth), 
                                                   ctypes.c_int(verticalPadding), ctypes.c_int(horizontalPadding),
                                                   ctypes.c_int(verticalStride), ctypes.c_int(horizontalStride))
    cudnnCheckStatus(status)
...
# Modified by Scott. It was out of date.
_libcudnn.cudnnActivationForward.restype = int
_libcudnn.cudnnActivationForward.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,
                                             ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
def cudnnActivationForward(handle, activationDesc, alpha, srcDesc, srcData, beta, destDesc, destData):
    """"
    ...
    #srcDataRef = ctypes.byref(srcData)
    #destDataRef = ctypes.byref(destData)

    status = _libcudnn.cudnnActivationForward(handle, activationDesc, alphaRef, srcDesc, srcData,
                                              betaRef, destDesc, destData)
    cudnnCheckStatus(status)
...
def cudnnCreateActivationDescriptor():
    ...
    status = _libcudnn.cudnnCreateActivationDescriptor(ctypes.byref(descriptor))
    cudnnCheckStatus(status)
...
def cudnnSetActivationDescriptor(activationDesc, mode, reluNanOpt, coef):

    status = _libcudnn.cudnnSetActivationDescriptor(activationDesc, mode, reluNanOpt, ctypes.c_double(coef))
    cudnnCheckStatus(status)


#_libcudnn.cudnnGetConvolutionForwardAlgorithm_7.restype = int
#_libcudnn.cudnnGetConvolutionForwardAlgorithm_7.argtypes = [ctypes.c_void_p,
#                                                            ctypes.c_void_p, ctypes.c_void_p,
#                                                            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int,
#                                                            ctypes.c_void_p, ctypes.c_void_p]
#def cudnnGetConvolutionForwardAlgorithm_7(handle, xDesc, wDesc, convDesc, yDesc, requestedAlgoCount):
#
#    returned_algo_count = ctypes.c_int()
#    algo = ctypes.c_int()
#
#    status = _libcudnn.cudnnGetConvolutionForwardAlgorithm_7(handle, srcDesc, wDesc,
#                                                             convDesc, yDesc, requestedAlgoCount,
